...
#ifdef VENDOR_EDIT
//Liujie.Xie@TECH.Kernel.Sched, 2020/08/14, add for setting critical thread with light load to rt
#include <uapi/linux/sched/types.h>

#define MALI_THREAD_NAME "mali-cmar-backe"
#define LAUNCHER_THREAD_NAME "m.oppo.launcher"

static inline int oplus_thread_turbo(struct task_struct *p)
{
	...
	if (strstr(grp_leader->comm, LAUNCHER_THREAD_NAME) && strstr(p->comm, MALI_THREAD_NAME)) {
		int ret = sched_setscheduler_nocheck(p, SCHED_FIFO, &param);
		return ret;
	}
