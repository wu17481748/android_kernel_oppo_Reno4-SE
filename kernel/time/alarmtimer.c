...
static enum hrtimer_restart alarmtimer_fired(struct hrtimer *timer)
{
	...
	if (alarm->type == ALARM_BOOTTIME) {
		...
		if(atomic_read(&alarm_atomic) || atomic_read(&alarm_sleep_busy_atomic)) {
			...
			if (alarm->function) {
				pr_info("%s.: type=%d, count=%lld, wakeup count=%lld, func=%pf, owner=%s\n", __func__, alarm->type, alarm_count, wakeup_source_count_rtc, alarm->function, alarm->comm); //log diff, better for log filter
			}
		} else {
			if (alarm->function) {
...
void alarm_start(struct alarm *alarm, ktime_t start)
{
	...
	spin_unlock_irqrestore(&base->lock, flags);
#ifdef VENDOR_EDIT
	memcpy(alarm->comm, current->comm, TASK_COMM_LEN);//get this alarm's owner
#endif /* VENDOR_EDIT */
