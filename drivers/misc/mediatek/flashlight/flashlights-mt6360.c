...
#ifdef VENDOR_EDIT
/*Henry.Chang@Camera.Driver add for 19165 torch duty 20190803*/
...
static const int mt6360_current[MT6360_LEVEL_NUM] = {
	100, 200, 250, 300, 400, 500, 600, 700, 800, 900, 1000, 1100
};
...
/* 0x74:bit7=1, 25+6.25*n; bit7=0, 50+12.5*n */
static const unsigned char mt6360_strobe_level[MT6360_LEVEL_FLASH] = {
	0x04, 0x0C, 0x10, 0x14, 0x1C, 0x24, 0x2C, 0x34, 0x3C, 0x44, 0x4C, 0x54
};
#else
...
// Wenjun.Wu@Cam.drv, 20200218, add for 19131
...
static const unsigned char mt6360_torch_level[MT6360_LEVEL_TORCH] = {
	0x00, 0x01, 0x02, 0x04, 0x07, 0x0A, 0x0C, 0x0E, 0x10, 0x12,
	0x14, 0x16, 0x18, 0x1A, 0x1C, 0x1E
};
...
static int mt6360_set_level_ch1(int level)
{
	...
	if (!mt6360_is_torch(level)) {
		if (is_project(OPPO_19165)) {
			...
			||is_project(OPPO_20001)||is_project(OPPO_20002)||is_project(OPPO_20003)||is_project(OPPO_20200)){
			flashlight_set_torch_brightness(
...
static int mt6360_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_TIME_OUT_TIME_MS:
		...
		#ifdef VENDOR_EDIT
		/* Feiping.Li@Cam.Drv, 20200114, modify for 19040 dual flash */
		if (is_project(OPPO_19040)) {
