...
#define OPPO_HVDCP_DETECT_TO_DETACH_TIME 90

#define HVDCP_EXIT_NORMAL	0
#define HVDCP_EXIT_ABNORMAL	1

#endif /*CONFIG_OPPO_HVDCP_SUPPORT*/
...
/* Kaili.Lu@BSP.CHG.Basic, 2019/09/30,  Add for HVDCP charging */
		if (true == is_mtksvooc_project) {
			...
			printk(KERN_ERR "!!! %s: the hvdcp_detach_time:%lu %lu %d %d\n", __func__, mpci->hvdcp_detach_time, mpci->hvdcp_detect_time, OPPO_HVDCP_DETECT_TO_DETACH_TIME, mpci->hvdcp_cfg_9v_done);
			if (mpci->hvdcp_cfg_9v_done && (mpci->hvdcp_detach_time - mpci->hvdcp_detect_time <= OPPO_HVDCP_DETECT_TO_DETACH_TIME)) {
...
#ifdef VENDOR_EDIT
/* tongfeng.huang@BSP.CHG.Basic, 2020/07/28,  Add for wake update wokr charging when bc1.2 done */
...
#ifdef CONFIG_OPPO_CHARGER_MTK6853
...
void mt6360_enable_hvdcp_detect(void)
{
	...
	/*Lukaili@BSP.CHG.Basic, 2020/06/16, Add for qc compatible for vooc project*/
	dev_err(oppompci->dev, "%s: enable hvdcp detect is_mtksvooc_project = %d %d\n", __func__, is_mtksvooc_project, oppompci->hvdcp_exit_stat);
	if (true == is_mtksvooc_project) {
		if (HVDCP_EXIT_NORMAL == oppompci->hvdcp_exit_stat){
			goto enable_hvdcp;
		} else {
			//disable hvdcp
			ret = mt6360_pmu_reg_write(oppompci->mpi, HVDCP_DEVICE_TYPE, 0x0);
			dev_err(oppompci->dev, "%s: HVDCP_EXIT_ABNORMAL not enable hvdcp \n", __func__);
			return;
		}
	}
...
int mt6360_get_flashlight_temperature(int *temp)
{
	...
	if (oppompci) {
		...
		pr_err("%s oppompci NULL\n", __func__);
		return -1;
	}
	ret = iio_read_channel_processed(oppompci->channels[MT6360_ADC_TS], temp);
	//printk(KERN_DEBUG "%s: raw flashlight temp=%d\n", __func__, *temp);
	if (ret < 0) {
		dev_info(oppompci->dev, "%s: fail(%d)\n", __func__, ret);
		return ret;
	}
	return 0;
}
