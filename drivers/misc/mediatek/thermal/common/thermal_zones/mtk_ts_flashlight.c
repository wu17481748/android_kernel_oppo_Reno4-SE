...
static int mtk_flashlight_get_temp(struct thermal_zone_device *thermal, int *t)
{
	...
	if (!kernelmode) {
		*t = INVALID_TEMP;
		mtk_flashlight_printk("[%s] zone disabled, return %d\n", __func__, INVALID_TEMP);
		return 0;
	}
	mtk_flashlight_printk("[%s] Enter\n", __func__);
	ret = mt6360_get_flashlight_temperature(&temp_uv);

	mtk_flashlight_printk("[%s] T_flashlight get_temp_status %d, temp_uv, %d\n", __func__, ret, temp_uv);
	pr_err("[%s] T_flashlight get_temp_status %d, temp_uv, %d\n", __func__, ret, temp_uv);

	real_temp = FLASHLIGHT_Temperature_Table[pre_temp_idx].NTC_Temp * 1000;

	if (temp_uv < FLASHLIGHT_Temperature_Table[pre_temp_idx].TemperatureR) {
		for (i = pre_temp_idx + 1; i < TEMPERATURE_TBL_SIZE; i++) {
			if (temp_uv < FLASHLIGHT_Temperature_Table[i].TemperatureR)
				real_temp = FLASHLIGHT_Temperature_Table[i].NTC_Temp * 1000;
			else
				break;
		}
		pre_temp_idx = i - 1;
	} else {
		for (i = pre_temp_idx - 1; i > 0; i--) {
			if (temp_uv > FLASHLIGHT_Temperature_Table[i].TemperatureR)
				real_temp = FLASHLIGHT_Temperature_Table[i].NTC_Temp * 1000;
			else
				break;
	...
	*t = real_temp;
	...
	pr_err("[%s] T_flashlight real_temp, %d\n", __func__, real_temp);
...
#if 0
static void mtkts_flashlight_cancel_thermal_timer(void)
{
	/* cancel timer */
	/*pr_debug("mtkts_flashlight_cancel_thermal_timer\n"); */
...
static void mtkts_flashlight_start_thermal_timer(void)
{
	/*pr_debug("mtkts_flashlight_start_thermal_timer\n"); */
...
static int __init mtk_flashlight_init(void)
{
	...
	switch (pro_no) {
		...
		case OPPO_20002:
		case OPPO_20003:
			kernelmode = THERMAL_DEVICE_ENABLED;
			break;
		case OPPO_20075:
		case OPPO_20076:
			kernelmode = THERMAL_DEVICE_DISABLED;
			break;
		default:
			kernelmode = THERMAL_DEVICE_DISABLED;
			break;
