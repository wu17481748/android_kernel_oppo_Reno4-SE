...
#ifdef VENDOR_EDIT
//Feiping.Li@Cam.Drv, 20200108, add for eeprom check
char gOtpCheckdata[7][40];
#if 1	//def CONFIG_OPPO_DUAL_CHARGER_SUPPORT
//Wenjun.Wu@Cam.Drv, 20200213, add for charge status change
	...
	PK_INFO("%s secure state %d", __func__,
		...
		if (ret != ERROR_NONE) {
			...
			if (is_project(OPPO_19131)||is_project(OPPO_19132)||is_project(OPPO_19133)||is_project(OPPO_19420)
				||is_project(OPPO_20001)||is_project(OPPO_20075)||is_project(OPPO_20076)||is_project(OPPO_20002)
				||is_project(OPPO_20003)||is_project(OPPO_20200)){
...
int imgsensor_set_driver(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (is_project(OPPO_19040)) {
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
			imgsensor_custom_config_19040[psensor_inst->sensor_idx].i2c_dev);
	} else {
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	#ifdef VENDOR_EDIT
	/*Henry.Chang@Camera.Driver add for 18073 ModuleInfo 20190530*/
