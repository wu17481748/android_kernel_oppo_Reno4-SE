...
/*Caohua.Lin@Camera, fix sensor data small than grab window issue,20200710*/
static kal_uint32 pre_frame_length;
static uint8_t first_frame_flag = 0;
...
#ifndef VENDOR_EDIT
/*Caohua.Lin@Cam.Drv, remove for s5k4h7 porting for latour, 20200514*/
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{3264, 2448, 0,   0, 3264, 2448,  816,  612,   0, 0,  640,  480, 0, 0,  640,  480},	//hight speed video
...
static kal_uint16 s5k4h7_custom1_setting[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static kal_uint16 s5k4h7_custom2_setting[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static kal_uint16 s5k4h7_custom3_setting[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static kal_uint16 s5k4h7_custom4_setting[] = {
	0x0100, 0x00,
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
...
static kal_uint16 s5k4h7_custom5_setting[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static void custom1_setting(void)
{
	LOG_INF("E");
	first_frame_flag = 0;
	table_write_cmos_sensor(s5k4h7_custom1_setting,
		sizeof(s5k4h7_custom1_setting) / sizeof(kal_uint16));
}
...
static void custom2_setting(void)
{
	LOG_INF("E");
	first_frame_flag = 0;
	table_write_cmos_sensor(s5k4h7_custom2_setting,
		sizeof(s5k4h7_custom2_setting) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	LOG_INF("E");
	first_frame_flag = 0;
	table_write_cmos_sensor(s5k4h7_custom3_setting,
		sizeof(s5k4h7_custom3_setting) / sizeof(kal_uint16));
}
...
static void custom4_setting(void)
{
	LOG_INF("E");
	first_frame_flag = 0;
	table_write_cmos_sensor(s5k4h7_custom4_setting,
		sizeof(s5k4h7_custom4_setting) / sizeof(kal_uint16));
}
...
static void custom5_setting(void)
{
	LOG_INF("E");
	first_frame_flag = 0;
	table_write_cmos_sensor(s5k4h7_custom5_setting,
		sizeof(s5k4h7_custom5_setting) / sizeof(kal_uint16));
}

static kal_uint8 gS5K4H7_SN[CAMERA_MODULE_SN_LENGTH];
static void read_sensor_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gS5K4H7_SN[0];
	write_cmos_sensor_8(0x0A02, 0x16);    // page 22
	write_cmos_sensor_8(0x0A00, 0x01);
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		char pusendcmd[2] = {0x0A , (char)((0x24 + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0x20);
		LOG_INF("S5K4H7_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gS5K4H7_SN[idx]);
	}
}

#define  CAMERA_MODULE_INFO_LENGTH  (8)
static kal_uint8 gS5K4H7_CamInfo[CAMERA_MODULE_INFO_LENGTH];
static void read_sensor_CamInfo(void)
{
	kal_uint16 idx = 0;
	...
	for (idx = 0; idx <12; idx++) {
		char pusendcmd[2] = {0x0A , (char)((0x04 + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0x20);
		LOG_INF("S5K4H7_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	gS5K4H7_CamInfo[0] = get_byte[0];
	gS5K4H7_CamInfo[1] = get_byte[1];
	gS5K4H7_CamInfo[2] = get_byte[6];
	gS5K4H7_CamInfo[3] = get_byte[7];
	gS5K4H7_CamInfo[4] = get_byte[8];
	gS5K4H7_CamInfo[5] = get_byte[9];
	gS5K4H7_CamInfo[6] = get_byte[10];
	gS5K4H7_CamInfo[7] = get_byte[11];
}
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_MODULE_INFO:
		LOG_INF("S5K4H7 GET_MODULE_CamInfo:%d %d\n", *feature_para_len, *feature_data_32);
		*(feature_data_32 + 1) = (gS5K4H7_CamInfo[1] << 24)
					| (gS5K4H7_CamInfo[0] << 16)
					| (gS5K4H7_CamInfo[3] << 8)
					| (gS5K4H7_CamInfo[2] & 0xFF);
		*(feature_data_32 + 2) = (gS5K4H7_CamInfo[5] << 24)
					| (gS5K4H7_CamInfo[4] << 16)
					| (gS5K4H7_CamInfo[7] << 8)
					| (gS5K4H7_CamInfo[6] & 0xFF);
		break;
	case SENSOR_FEATURE_GET_MODULE_SN:
		LOG_INF("S5K4H7 GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
		if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
			*(feature_data_32 + 1) = (gS5K4H7_SN[4*(*feature_data_32) + 3] << 24)
						| (gS5K4H7_SN[4*(*feature_data_32) + 2] << 16)
						| (gS5K4H7_SN[4*(*feature_data_32) + 1] << 8)
						| (gS5K4H7_SN[4*(*feature_data_32)] & 0xFF);
		break;
