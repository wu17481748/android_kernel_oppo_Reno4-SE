		...
		char *pcurr_idx)
{
	...
	if ((pwr_status == IMGSENSOR_HW_POWER_STATUS_ON) && is_project(OPPO_19040)) {
		pdev = phw->pdev[IMGSENSOR_HW_ID_GPIO];
		if (pdev->set != NULL) {
			PK_DBG("set gpio29 to enable pm8008");
			pdev->set(pdev->pinstance, IMGSENSOR_SENSOR_IDX_MAIN, IMGSENSOR_HW_PIN_PM8008_ENABLE, Vol_High);
		}
	}
	#endif
