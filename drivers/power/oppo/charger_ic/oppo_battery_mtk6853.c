...
void oppo_usbtemp_set_vbus_exist(bool exist);
...
static int charge_pump_mode = OPPO_DIVIDER_WORK_MODE_AUTO;
extern int oppo_set_divider_work_mode(int work_mode);
#endif
...
#ifdef VENDOR_EDIT
/* Kaili.Lu@BSP.CHG.Basic, 2020/10/13, Add for PD+SVOOC */
//int oppo_check_is_pd_svooc(void);
//static void register_oppo_pdsvooc_svid(struct work_struct *work) ;
static bool oppo_check_pdphy_ready(void) ;
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/11/09, sjc Add for charging */
	...
	if (is_vooc_project() == false) {
		...
		if (mt_get_charger_type() != CHARGER_UNKNOWN){
			...
			oppo_set_divider_work_mode(OPPO_DIVIDER_WORK_MODE_FIXED);
			...
			/* Lukaili@BSP.CHG.Basic, 2020/06/02, add for charge pump adjust frequency */
			oppo_set_divider_work_mode(charge_pump_mode);
			chr_err("charge_pump_mode = %d\n", charge_pump_mode);
			if (mtkhv_flashled_pinctrl.hv_flashled_support){
				...
				mutex_unlock(&(pinfo->usbswitch_lock));
				mtkhv_flashled_pinctrl.qc_online = false;
			}
...
int oppo_get_adapter_svid(void)
{
	int i = 0;
	...
	for (i = 0; i < svid_list.cnt; i++) {
		chr_err("svid[%d] = %d\n", i, svid_list.svids[i]);
		if (svid_list.svids[i] == OPPO_SVID) {
			if (g_oppo_chip) {
				g_oppo_chip->pd_svooc = true;
				chr_err("match svid and this is oppo adapter\n");
			}
		}
	}
...
static bool oppo_check_pdphy_ready(void){
	...
	return oppo_check_pd_state_ready();
}
...
static int oppo_mt6360_charging_current_write_fast(int chg_curr)
{
	...
	if (!g_oppo_chip->authenticate) {
		...
		printk(KERN_ERR "[OPPO_CHG][%s]:!g_oppo_chip->authenticate , charging_disable\n", __func__);
		return 0;
	}
...
static void oppo_mt6360_set_aicl_point(int vbatt)
{
	...
	if (!g_oppo_chip->authenticate) {
		printk(KERN_ERR "[OPPO_CHG][%s]:!g_oppo_chip->authenticate \n", __func__);
		return;
	}
...
static int oppo_mt6360_input_current_limit_write(int value)
{
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (g_oppo_chip->dual_charger_support && g_oppo_chip->slave_charger_enable == false) {
...
int oppo_set_input_current(int ma)
{
	if (!g_oppo_chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: g_oppo_chip is null!\n", __func__);
		return -1;
	}
...
static int oppo_mt6360_suspend_charger(void)
{
	...
	oppo_mt6360_input_current_limit_write(500);
		msleep(50);
...
bool oppo_mt_get_vbus_status(void)
{
	...
	if(g_oppo_chip && g_oppo_chip->vbatt_num == 2 && oppo_chg_get_otg_online()){
		return false;
	}
...
static int oppo_usb_switch_gpio_gpio_init(void)
{
	...
	if (pinfo) {
		mutex_init(&(pinfo->usbswitch_lock));
		printk(KERN_ERR "[OPPO_CHG][%s]: usbswitch_lock init!\n", __func__);
	}
...
static void get_chargeric_temp(struct charger_data *pdata)
{
	...
	if (pinfo && pinfo->support_ntc_01c_precision) {
		...
		if (i >= 0 && (i < ARRAY_SIZE(con_volt_19165) - 1)) {
...
{
	if (pinfo == NULL) {
		printk(KERN_ERR "[OPPO_CHG][%s]: pinfo == NULL!\n", __func__);
		return;
	}

	printk(KERN_ERR "[OPPO_CHG][%s]: vbus_exist: %d\n", __func__, exist);
...
void oppochg_update_usbtemp_input_limit_status(struct oppo_chg_chip *chip)
{
	static int limit_cur_cnt_r = 0;
	static int limit_cur_cnt_l = 0;
	static int recover_cur_cnt_r = 0;
	static int recover_cur_cnt_l = 0;
	static int usbtemp_status = SMART_EXIT_USBTEMP_LIMIT_CHARGER;
	static int usbtemp_status_pre = SMART_EXIT_USBTEMP_LIMIT_CHARGER;
	...
	if (chip) {
		...
		usbtemp_status_pre = usbtemp_status;
		if (RETRY_COUNT <= limit_cur_cnt_r || RETRY_COUNT <= limit_cur_cnt_l) {
			usbtemp_status = SMART_ENTER_USBTEMP_LIMIT_CHARGER;
		} else if (RETRY_COUNT <= recover_cur_cnt_r || RETRY_COUNT <= recover_cur_cnt_l){
			usbtemp_status = SMART_EXIT_USBTEMP_LIMIT_CHARGER;
		} else {
			//do nothing
		}

		if (usbtemp_status_pre != usbtemp_status || 0 == chip->cool_down) {
			oppo_smart_charge_by_cool_down(chip, usbtemp_status);
		}
	}
	return;
}
...
static int oppo_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop() && dischg_flag == false) {
		...
		if (is_vooc_project()) {
			oppochg_update_usbtemp_input_limit_status(chip);
		}
...
check_again:
			...
			wait_event_interruptible(oppo_usbtemp_wq,
				oppo_chg_get_vbus_status(chip) == true && oppo_chg_get_otg_online() == false);
		}
	}
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (is_vooc_project()) {
			switch (psp) {
				...
				case POWER_SUPPLY_PROP_FIXED_FREQ:
					if (val->intval) {
						charge_pump_mode = OPPO_DIVIDER_WORK_MODE_FIXED;
						oppo_set_divider_work_mode(OPPO_DIVIDER_WORK_MODE_FIXED);
					} else {
						charge_pump_mode = OPPO_DIVIDER_WORK_MODE_AUTO;
						if (g_oppo_chip->charger_exist == false) {
							oppo_set_divider_work_mode(charge_pump_mode);
						}
					}
					printk(KERN_ERR "[OPPO_CHG][%s]: intval [%d] charger_exist[%d] charge_pump_mode[%d]\n", __func__, val->intval, g_oppo_chip->charger_exist, charge_pump_mode);
					break;
				default:
					rc = oppo_battery_set_property(psy, psp, val);
					break;
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (is_vooc_project()) {
			switch (psp) {
				case POWER_SUPPLY_PROP_FIXED_FREQ:
					val->intval = OPPO_DIVIDER_WORK_MODE_FIXED == charge_pump_mode ? 1 : 0;
					break;
				case POWER_SUPPLY_PROP_EM_MODE:
					val->intval = em_mode;
					break;
...
int oppo_mt6360_pd_setup_forsvooc(void)
{
	int vbus_mv = VBUS_5V;
	int ibus_ma = IBUS_2A;
	int ret = -1;
...
int oppo_mt6360_pd_setup(void)
{
	...
	if (is_mtksvooc_project) {
		ret = oppo_mt6360_pd_setup_forsvooc();
	} else {
		cap.nr = 0;
		cap.pdp = 0;
		for (i = 0; i < ADAPTER_CAP_MAX_NR; i++) {
...
int oppo_chg_get_charger_subtype(void)
{
	...
	if (mt6360_get_hvdcp_type() == POWER_SUPPLY_TYPE_USB_HVDCP) {
		mtkhv_flashled_pinctrl.qc_online = true;
...
bool oppo_chg_check_vbus_change(const unsigned int checkpoint, const bool isVbusBoost)
{
...
int oppo_chg_set_qc_config_forsvooc(void)
{
	...
	if (!chip->calling_on && chip->charger_volt < 6500 && chip->soc < 90
		...
		msleep(300);
		oppo_chg_unsuspend_charger();
		if(oppo_chg_check_vbus_change(8600, true)) {
			ret = 0;
		}
		oppo_notify_hvdcp_detect_stat();
	} else {
		if (chip->charger_volt > 7500 &&
			...
			printk(KERN_ERR "%s: charger voltage=%d", __func__, chip->charger_volt);
			oppo_chg_unsuspend_charger();
			if(oppo_chg_check_vbus_change(5800, false)) {
				ret = 0;
			}
		}
...
void oppo_chg_set_camera_on(bool val)
{
	if (!g_oppo_chip) {
		...
		if (mtkhv_flashled_pinctrl.hv_flashled_support) {
			chr_err("%s: bc1.2_done = %d camera_on %d qc_online = %d\n", __func__, mtkhv_flashled_pinctrl.bc1_2_done, val, mtkhv_flashled_pinctrl.qc_online);
			if (mtkhv_flashled_pinctrl.bc1_2_done) {
				if (val) {
					...
					pinctrl_select_state(mtkhv_flashled_pinctrl.pinctrl, mtkhv_flashled_pinctrl.chgvin_enable);
					mutex_unlock(&(pinfo->usbswitch_lock));
					if (g_oppo_chip->charger_exist && POWER_SUPPLY_TYPE_USB_DCP == g_oppo_chip->charger_type && mtkhv_flashled_pinctrl.qc_online) {
...
struct oppo_chg_operations  mtk6360_chg_ops = {
	...
	.check_pdphy_ready = oppo_check_pdphy_ready,
};
//====================================================================//
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/11/09, sjc Add for shipmode stm6620 */
	...
	if (g_oppo_chip->charger_exist && is_vooc_project()) {
		  g_oppo_chip->chg_ops->input_current_write(500);
