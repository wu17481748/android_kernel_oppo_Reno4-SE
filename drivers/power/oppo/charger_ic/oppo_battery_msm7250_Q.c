...
extern void oppo_set_usb_status(int status);
extern void oppo_clear_usb_status(int status);
extern int oppo_get_usb_status(void);
				    ...
				    union power_supply_propval *val)
{
	...
	/* tongfeng.Huang@BSP.CHG.Basic, 2019/01/10,  Add for c-c cable detect as water */
		...
		if (chip && chip->vbatt_num == 2 && chip->chg_ops->get_charger_current){
			if(oppo_vooc_get_allow_reading()){
...
int smblib_get_pe_start(struct smb_charger *chg,
			       union power_supply_propval *val)
{
#if 0 //def VENDOR_EDIT		/*ZhangKun@BSP.CHG.Basicrm for pd detection hardrest during bootup*/
	/* Jacky.Zhuo@BSP.CHG.Basic, 2020/04/24,  Add for PD charger detection during bootup */
	u8 stat = 0x0;
	static int count = 5;
	smblib_read(chg, LEGACY_CABLE_STATUS_REG, &stat);
	smblib_err(chg, "LEGACY_CABLE_STATUS_REG status = 0x%x\n",stat);
	if((stat & 0x1) && (count > 0)) {
		count--;
		val->intval = true;
		smblib_err(chg, "Non-Compliant Type-A to Type-C cable, try PD detection!\n");
	}
	else
	{
		count = 5;
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/04/13, sjc Add for charging */
		...
		if(g_oppo_chip->pmic_spmi.smb5_chip->chg.pd_active)
			oppo_chg_wake_update_work();
#endif
	} else {
...
#ifdef VENDOR_EDIT  /* zhangkun@BSP.CHG.Basic, 2019/09/01, Add for Charger*/
...
void oppo_set_otg_switch_status_default(bool value)
{
	int rc;
	u8 stat;
	bool vbus_rising;
				...
				const union power_supply_propval *val)
{
	...
	if (chg->pr_swap_in_progress) {
		...
		if(oppo_ccdetect_check_is_gpio(chip) == true) { 
			level = gpio_get_value(chg->ccdetect_gpio);
#if 0	/*ZhangKun@BSP.CHG.Basicrm for pd detection hardrest during bootup*/
		...
		if((oppo_get_usb_status() & USB_TEMP_HIGH) != 0){
			reg_val = EN_SNK_ONLY_BIT;
		}
		/* enable DRP */
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
...
static void  get_usb_temp(struct oppo_chg_chip *chg)
{
	...
	if(g_oppo_chip->vbatt_num == 2){
		memcpy(volt_temp,con_volt_7250_svooc,sizeof(con_volt_7250_svooc));
	}else{
		memcpy(volt_temp,con_volt_7250,sizeof(con_volt_7250));
	}

	for (i = ARRAY_SIZE(volt_temp) - 1; i >= 0; i--) {
		if (volt_temp[i] >= chg->usbtemp_volt_r)
			break;
		else if (i == 0)
			break;
	}
	...
	for (i = ARRAY_SIZE(volt_temp) - 1; i >= 0; i--) {		
		if (volt_temp[i] >= chg->usbtemp_volt_l)
			break;
		else if (i == 0)
			break;
	}
			...
			if ((USB_57C <= chip->usb_temp_r && chip->usb_temp_r < USB_100C)
				...
				if (dischg_flag == false) {
					...
					if (count_r >= retry_cnt || count_l >= retry_cnt) {
						if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable) && 
							!(get_project() == 19101 && strcmp("T0",get_PCB_Version()) == 0)) {
							dischg_flag = true;
...
/*power off charging without usbtemp detect
	if (oppo_usbtemp_check_is_support() == true
		&& qpnp_is_power_off_charging() == false){
*/
	if (oppo_usbtemp_check_is_support() == true){
		wake_up_interruptible(&oppo_usbtemp_wq);
	}
...
int pm7250b_bob_regulator_get_mode(unsigned int *mode)
{
	...
	if (!g_oppo_chip) {
		printk(KERN_ERR "pm7250b_bob_regulator_get_mode: g_oppo_chip NULL\n");
		return -1;
	}
	chg = &g_oppo_chip->pmic_spmi.smb5_chip->chg;
...
/* tongfeng.Huang@BSP.CHG.Basic, 2018/11/07,  Delete for charging */
	...
	case POWER_SUPPLY_PROP_SET_SHIP_MODE:
		...
		if(oppo_ship_check_is_gpio(g_oppo_chip) == false)
