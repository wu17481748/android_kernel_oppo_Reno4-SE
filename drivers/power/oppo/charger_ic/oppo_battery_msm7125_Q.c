...
void oppo_set_oppochg_pd_sdp(bool value);
static int oppo_chg_get_ibus(void);
void oppo_chg_set_chargerid_to_mcu_work(struct work_struct *work);
void oppo_chg_reset_mcu_work(struct work_struct *work);
...
#define OPPO_CHARGERID_DEFAULT 950
...
void oppo_set_oppochg_pd_sdp(bool value)
{
	oppochg_pd_sdp =value;
	if(value == true && g_oppo_chip
		&&  g_oppo_chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP
		&&	POWER_SUPPLY_TYPE_USB_PD == g_oppo_chip->pmic_spmi.smb5_chip->chg.real_charger_type){
		g_oppo_chip->charger_type = POWER_SUPPLY_TYPE_USB;
	}
}
...
static int oppo_chg_get_charger_type(void)
{
	u8 apsd_stat;
	int rc;
	struct smb_charger *chg = NULL;
...
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for charging */
	if (vbus_rising) {
		...
		if (g_oppo_chip && g_oppo_chip->ui_otg_switch != g_oppo_chip->otg_switch)
			oppo_set_otg_switch_status(g_oppo_chip->ui_otg_switch);
		cancel_delayed_work_sync(&chg->chg_monitor_work);
	}
#endif
#ifdef VENDOR_EDIT
...
/* Gang.Yan@BSP.CHG.Basic, 2020/04/08, Add for pd charging */
		if(g_oppo_chip && g_oppo_chip->pmic_spmi.smb5_chip->chg.pd_active)
			oppo_chg_wake_update_work();
#endif
	} else {
#ifdef VENDOR_EDIT
...
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for charging */
	...
	INIT_DELAYED_WORK(&chg->set_chargerid_to_mcu_work, oppo_chg_set_chargerid_to_mcu_work);
	INIT_DELAYED_WORK(&chg->reset_mcu_work, oppo_chg_reset_mcu_work);
#endif
...
int oppo_mcu_id_gpio_init(struct oppo_chg_chip *chip)
{
...
/*power off charging without usbtemp detect
	if(oppo_usbtemp_check_is_gpio(g_oppo_chip) == true &&  qpnp_is_power_off_charging() != true)
*/
...
int oppo_turn_usbtemp_to_volt(int temp,const struct vadc_map_temp *table, int length)
{
	int volt = -1;
	int i = 0;
	for(i=0;i<length; i++){
...
void oppo_get_usbtemp_volt(struct oppo_chg_chip *chip)
{
	...
	usbtemp_volt = oppo_turn_usbtemp_to_volt(usbtemp,origin_table,len);
	if (rc < 0) {
...
int oppo_get_usb_status(void)
{
	if( g_oppo_chip->usb_status == USB_TEMP_HIGH){
		return g_oppo_chip->usb_status;
	} else {
		return usb_status;
	}
}
extern int oppo_usbtemp_monitor_common(void *data);
static void oppo_usbtemp_thread_init(void)
{
	oppo_usbtemp_kthread =
			kthread_run(oppo_usbtemp_monitor_common,g_oppo_chip, "usbtemp_kthread");
...
void oppo_wake_up_usbtemp_thread(void)
{
	if (oppo_usbtemp_check_is_support() == true){
		wake_up_interruptible(&g_oppo_chip->oppo_usbtemp_wq);
	}
}
...
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Modify for charging */
		...
		if (g_oppo_chip && g_oppo_chip->vbatt_num == 2) {
			if (oppo_chg_get_charger_type() == POWER_SUPPLY_TYPE_USB
				|| oppo_chg_get_charger_type() == POWER_SUPPLY_TYPE_USB_CDP) {
				val->intval = 1;
			}
		}
#else
...
/* Ji.Xu PSW.BSP.CHG  2018-07-23  Save battery capacity to persist partition */
	...
	case POWER_SUPPLY_PROP_SOC_NOTIFY_READY:
		val->intval = g_oppo_chip->healthd_ready;
		break;
	case POWER_SUPPLY_PROP_RESTORE_SOC:
		if (g_oppo_chip->healthd_ready)
...
/* Gang.Yan@BSP.CHG.Basic, 2020/05/09,   add for cts */
		if (g_oppo_chip) {
			val->intval = g_oppo_chip->batt_rm * 1000;
		}
#endif
		break;
...
/* lizhijie@PSW.BSP.CHG.Basic, 2018/04/19, sjc Add for CTS */
	...
	default:
#ifdef VENDOR_EDIT
/*lizhijie@BSP.CHG.Basic. 2020/02/25 lzj add for oppo own battery props*/
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_INFO_ID:
		...
		break;
	case POWER_SUPPLY_PROP_SOC_NOTIFY_READY:
		g_oppo_chip->healthd_ready = val->intval;
...
bool oppo_usb_or_otg_is_present(void)
{
	int rc = 0;
	u8 stat = 0;
...
void oppo_chg_set_chargerid_to_mcu_work(struct work_struct *work){
	...
	if((oppo_chg_get_charger_subtype() !=  CHARGER_SUBTYPE_QC) && (smbchg_get_chargerid_switch_val() == 0)){
		pr_err("[OPPO_CHG] NO QC ,set switch to MCU");
		smbchg_set_chargerid_switch_val(1);
	}
}
extern  void oppo_chg_set_input_current_limit(struct oppo_chg_chip *chip);
void oppo_chg_reset_mcu_work(struct work_struct *work){
	struct smb_charger *chg = NULL;
	struct oppo_chg_chip *chip = g_oppo_chip;
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	is_fastchg = oppo_chg_show_vooc_logo_ornot();
	if(!is_fastchg && oppo_chg_get_charger_subtype() != CHARGER_SUBTYPE_PD && oppo_chg_get_charger_subtype() != CHARGER_SUBTYPE_QC){
		pr_err("[OPPO_CHG] reset_MCU_ready");
		chip->chg_ops->input_current_write(500);
		oppo_chg_suspend_charger();
...
int oppo_chg_enable_qc_detect(void)
{
	...
	pr_err("[OPPO_CHG] SHCEDULE set_chargerid_to_mcu_work ");
	schedule_delayed_work(&chg->set_chargerid_to_mcu_work, msecs_to_jiffies(JUDGE_SUBTYPE_QC));
	pr_err("[OPPO_CHG] SHCEDULE RESET_MCU_WORK ");
	schedule_delayed_work(&chg->reset_mcu_work, msecs_to_jiffies(RESET_MCU));
	return ret;
}
...
struct oppo_chg_operations  smb5_chg_ops = {
	...
	.get_usbtemp_volt = oppo_get_usbtemp_volt,
	.set_typec_sinkonly = oppo_set_typec_sinkonly,
	.oppo_usbtemp_monitor_condition = oppo_usb_or_otg_is_present,
	.get_ibus = oppo_chg_get_ibus ,
};
#endif /* VENDOR_EDIT */

EXPORT_SYMBOL(oppo_set_typec_sinkonly);
EXPORT_SYMBOL(oppo_get_usbtemp_volt);
EXPORT_SYMBOL(oppo_usb_or_otg_is_present);
...
//Gang.Yan add for usbtemp 
		oppo_chip->con_volt = con_volt_7125;
		oppo_chip->con_temp = con_temp_7125;
		oppo_chip->len_array = ARRAY_SIZE(con_volt_7125);
#endif
...
#ifdef VENDOR_EDIT
/*lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for charging*/
	if (oppo_chg_get_charger_subtype() == CHARGER_SUBTYPE_PD) {
	//	oppo_pdo_select(5000, 2000);
	}
#endif
