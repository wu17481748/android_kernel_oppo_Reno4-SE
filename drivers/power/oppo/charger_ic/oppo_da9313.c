...
/* Lukaili@BSP.CHG.Basic, 2020/06/17, Add for charging */
static int oppo_chgpump_gpio_init(struct chip_da9313 *chip)
{
...
static int oppo_chgpump_id_dt(struct chip_da9313 *chip)
{
	int rc = 0;
	...
	if (chip->chgpump_gpio <= 0) {
		...
		if (gpio_is_valid(chip->chgpump_gpio)) {
			...
			if (rc) {
				...
				rc = oppo_chgpump_gpio_init(chip);
				if (rc)
