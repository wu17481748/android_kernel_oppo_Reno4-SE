...
#ifdef VENDOR_EDIT
/*wangdengwen@oppo.com, 2020/05/28, Add for chargeric temp */
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/11/09, sjc Add for charging */
	...
	if (mt_get_charger_type() != CHARGER_UNKNOWN) {
		...
		if (g_oppo_chip && g_oppo_chip->dual_charger_support == true) {
...
static int oppo_mt6360_input_current_limit_write(int value)
{
	...
	if (g_oppo_chip->dual_charger_support
		&& bq25910_is_detected() == true
		&& g_oppo_chip->slave_charger_enable == false) {
	...
	if (g_oppo_chip->chg_ops->oppo_chg_get_pd_type) {
		if (g_oppo_chip->chg_ops->oppo_chg_get_pd_type() == true) {
			...
			if (rc >= 0 && ibus_ma >= 500 && ibus_ma < g_oppo_chip->limits.pd_input_current_charger_ma && value > ibus_ma) {
				value = ibus_ma;
...
static int oppo_mt6360_suspend_charger(void)
{
	...
	if (chip && chip->dual_charger_support == true
			&& bq25910_is_detected() == true) {
		rc = bq25910_suspend_charger();
		if (rc < 0) {
			chg_debug("suspend sub charging fail\n");
		}
	}
...
static int oppo_mt6360_unsuspend_charger(void)
{
	...
	if (chip && chip->dual_charger_support == true
		...
		rc = bq25910_unsuspend_charger();
		if (rc < 0) {
			chg_debug("unsuspend sub charging fail\n");
		}
	}
...
static bool is_support_chargerid_check(void)
{
	...
	if(pinfo != NULL && pinfo->chargerid_disable){
		return false;
	}else{
		return true;
	}
...
static int mt_get_chargerid_volt(void)
{
	...
	if (is_support_chargerid_check() == true) {
		if(pinfo != NULL && pinfo->chargerid_vol_disable != true){
...
static void oppo_get_chargeric_temp_volt(struct charger_data *pdata)
{
	...
	chargeric_temp_volt = mt_vadc_read(1, Channel_14);//get the typec tem adc volt
...
static void get_chargeric_temp(struct charger_data *pdata)
{
	...
	for (i = ARRAY_SIZE(g_Ntc104eVol) - 1; i >= 0; i--) {
		if (g_Ntc104eVol[i] >= pdata->chargeric_temp_volt)
			break;
		else if (i == 0)
			break;
	}
	pdata->chargeric_temp= g_Ntc104eTemp[i];
...
static int oppo_usbtemp_dischg_action(struct oppo_chg_chip *chip)
{
	...
	if (g_oppo_chip && g_oppo_chip->dual_charger_support
			&& bq25910_is_detected() == true)
...
static int oppo_mt6360_pd_setup(void)
{
	...
	if(!chip) {
		printk(KERN_ERR "%s:g_oppo_chip not ready\n", __func__);
		return -1;
	}

	if (chip->dual_charger_support == false || bq25910_is_detected() == false) {
		ibus_ma = chip->limits.pd_input_current_charger_ma;
		...
		if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90
			&& chip->temperature <= 400 && chip->cool_down_force_5v == false) {
		 	ret = oppo_pdc_setup_9v2a(9000, 2000);
		 	printk(KERN_ERR "%s: vbus[%d], ibus[%d]\n", __func__, 9000, 2000);
		} else {
			if (chip->charger_volt > 7500 &&
				...
				ret = oppo_pdc_setup_9v2a(5000, 3000);
		 		printk(KERN_ERR "%s: vbus[%d], ibus[%d]\n", __func__, 5000, 3000);
			}
		}
	}
...
int oppo_chg_get_charger_subtype(void)
{
	...
	if (pinfo->pd_type == PD_CONNECT_PE_READY_SNK ||
		pinfo->pd_type == PD_CONNECT_PE_READY_SNK_PD30 ||
		pinfo->pd_type == PD_CONNECT_PE_READY_SNK_APDO)
...
int oppo_chg_set_qc_config(void)
{
	...
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90
		...
		if(oppo_chg_check_vbus_change(7600, true)) //icl checkpoint is 7600
			ret = 0;
	} else {
		if (chip->charger_volt > 7500 &&
			(chip->calling_on || chip->camera_on || chip->soc >= 90 || chip->batt_volt >= 4450
			...
			if(oppo_chg_check_vbus_change(5800, false)) //icl checkpoint is 5800
				ret = 0;
		}
	}
...
{
	if (oppo_check_hvdcp_support() == true)
...
static void mt6360_step_charging_work(struct work_struct *work)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__NORMAL) {
		...
		if (oppo_get_vbatt_higherthan_xmv() == false) {
...
struct oppo_chg_operations  mtk6360_chg_ops = {
	...
	.enable_qc_detect = oppo_chg_enable_hvdcp_detect,
};
//====================================================================//
#ifdef VENDOR_EDIT
