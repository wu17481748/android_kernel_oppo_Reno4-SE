...
void oppo_chg_set_input_current_limit(struct oppo_chg_chip *chip);
...
int oppo_gauge_dump_register(void);
#ifdef  CONFIG_FB
...
void oppo_chg_ui_soc_decimal_deinit(void);

static void oppo_chg_show_ui_soc_decimal(struct work_struct *work);
               ...
               const char __user *buf, size_t len, loff_t *data)
{
	...
	if (buffer[0] == '0') {
		...
		pr_err("test boot_completed ok, no selinux\n");
	}
		...
		char __user *buff, size_t count, loff_t *off)
{
	...
	if (chip->vooc_show_ui_soc_decimal) {
		svooc_is_control_by_vooc = (chip->vbatt_num == 2 && oppo_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC);
		if(svooc_is_control_by_vooc != true && chip->calculate_decimal_time == 0 && oppo_chg_show_vooc_logo_ornot() == true
			&& chip->boot_completed) {
			cancel_delayed_work_sync(&chip->ui_soc_decimal_work);
			oppo_chg_ui_soc_decimal_init();
	...
	pr_err("APK successful, %d,  %d, %d \n", val, chip->calculate_decimal_time, oppo_chg_show_vooc_logo_ornot());
	len = sprintf(page, "%s", read_data);
...
static int init_ui_soc_decimal_proc(struct oppo_chg_chip *chip)
{
	int ret = 0;
	struct proc_dir_entry *prEntry_tmp = NULL;
...
void oppo_chg_ui_soc_decimal_init(void)
{
	...
	if (oppo_vooc_get_fastchg_started() == true) {
		chip->batt_rm =  oppo_gauge_get_prev_remaining_capacity() * chip->vbatt_num;
		chip->batt_fcc =  oppo_gauge_get_prev_batt_fcc() * chip->vbatt_num;
	} else {
		chip->batt_rm =  oppo_gauge_get_remaining_capacity() * chip->vbatt_num;
		chip->batt_fcc =  oppo_gauge_get_batt_fcc() * chip->vbatt_num;
	}
	pr_err("[oppo_chg_ui_soc_decimal_init]!!!soc:%d", (int)((chip->batt_rm * 10000) / chip->batt_fcc));
	...
	pr_err("[oppo_chg_ui_soc_decimal_init]!!! 2VBUS ui_soc_decimal:%d", chip->ui_soc_integer + chip->ui_soc_decimal);
...
void oppo_chg_ui_soc_decimal_deinit(void)
{
	...
	pr_err("[oppo_chg_ui_soc_decimal_deinit] ui_soc:%d", chip->ui_soc);
...
static void oppo_chg_show_ui_soc_decimal(struct work_struct *work)
{
	...
	pr_err("[oppo_chg_show_ui_soc_decimal] start test icharging \n");
	if (oppo_vooc_get_fastchg_started() == true) {
		chip->batt_rm =  oppo_gauge_get_prev_remaining_capacity() * chip->vbatt_num;
		chip->batt_fcc =  oppo_gauge_get_prev_batt_fcc() * chip->vbatt_num;
		chip->icharging = oppo_gauge_get_batt_current();
	...
	if(icharging > 0) {
		...
		pr_err("[oppo_chg_show_ui_soc_decimal] icharging = %d, batt_fcc :%d", chip->icharging, chip->batt_fcc);
	} else {
	...
	pr_err("[oppo_chg_show_ui_soc_decimal]chip->ui_soc_decimal+chip_ui_soc: %d , speed: %d, soc :%d\n ",
	...
	if(chip->calculate_decimal_time<= MAX_UI_DECIMAL_TIME) {
		...
		oppo_chg_ui_soc_decimal_deinit();
	}
}
...
int oppo_chg_parse_charger_dt(struct oppo_chg_chip *chip)
{
	...
	if (rc) {
		chip->ui_soc_decimal_speedmin = 20;
	}
...
void oppo_chg_variables_reset(struct oppo_chg_chip *chip, bool in)
{
	if (in) {
		...
		if(!oppo_chg_show_vooc_logo_ornot()) {
			if(chip->decimal_control) {
				...
				oppo_chg_ui_soc_decimal_deinit();
				pr_err("[oppo_chg_variables_reset]cancel last_decimal_ui_soc:%d", chip->last_decimal_ui_soc);
			}
...
static void oppo_chg_variables_init(struct oppo_chg_chip *chip)
{
	...
	chip->limits.usbtemp_input_current_vooc_ma_normal = chip->limits.input_current_vooc_ma_normal;
#ifdef CONFIG_OPPO_CHARGER_MTK
	chip->usb_online = false;
	chip->otg_online = false;
#else
...
static void oppo_chg_smooth_to_soc(struct oppo_chg_chip *chip)
{
	...
	pr_err("[oppo_chg_smooth_to_soc] enter the func");
	...
	if (chip->charger_exist && chip->batt_exist
		...
		if (chip->smooth_soc == chip->ui_soc) {
			...
			if (soc_down_count >= soc_down_limit) {
				soc_down_count = 0;
				if(oppo_chg_show_vooc_logo_ornot() == false
...
static void oppo_chg_other_thing(struct oppo_chg_chip *chip)
{
	...
	oppo_chg_debug_chg_monitor(chip);
...
static void oppo_chg_pd_config(struct oppo_chg_chip *chip)
{
	...
	if (chip->pd_chging == false && chip->chg_ops->oppo_chg_get_pd_type() == true
			&& oppo_chg_show_vooc_logo_ornot() == false) {
...
int oppo_chg_get_icharging(void)
{
	if (!g_charger_chip) {
		return 4000;
	} else {
...
void oppo_chg_soc_update(void)
{
	...
	oppo_chg_update_ui_soc(g_charger_chip);
	oppo_chg_debug_set_soc_info(g_charger_chip);
}
...
int oppo_chg_get_cool_down_status(void)
{
	...
	if (!g_charger_chip) {
		...
		ret = g_charger_chip->cool_down;
		if (g_charger_chip->smart_charge_user == SMART_CHARGE_USER_USBTEMP) {
			if (g_charger_chip->cool_down)
				ret = (g_charger_chip->usbtemp_cool_down < g_charger_chip->cool_down) ? g_charger_chip->usbtemp_cool_down : g_charger_chip->cool_down;
			else
				ret = g_charger_chip->usbtemp_cool_down;
		} else {
			ret = g_charger_chip->cool_down;
		}

		charger_xlog_printk(CHG_LOG_CRTI, "cool_down = [%d %d %d %d]\n", g_charger_chip->cool_down, g_charger_chip->usbtemp_cool_down, ret, g_charger_chip->smart_charge_user);
		return ret;
	}
}
...
void oppo_smart_charge_by_cool_down(struct oppo_chg_chip *chip, int val)
{
	...
	if (val & SMART_NORMAL_CHARGER_9V1500mA) {                                   //9V1.5A
		chip->limits.input_current_cool_down_ma = OPPO_CHG_1500_CHARGING_CURRENT;
	...
	if (val & SMART_NORMAL_CHARGER_2000MA) {                                   //5V2A
		chip->limits.input_current_cool_down_ma = OPPO_CHG_2000_CHARGING_CURRENT;
	...
	if (val & SMART_NORMAL_CHARGER_1200MA) {
		chip->limits.input_current_cool_down_ma = OPPO_CHG_1200_CHARGING_CURRENT;
	...
	if (val & SMART_NORMAL_CHARGER_900MA) {
		chip->limits.input_current_cool_down_ma = OPPO_CHG_900_CHARGING_CURRENT;
	...
	if (val & SMART_NORMAL_CHARGER_500MA) {
		chip->limits.input_current_cool_down_ma = OPPO_CHG_500_CHARGING_CURRENT;
	...
	if (val & SMART_COMPATIBLE_VOOC_CHARGER_CURRENT_BIT0) {
		chip->limits.input_current_cool_down_ma = OPPO_CHG_1800_CHARGING_CURRENT;
	...
	if (SMART_ENTER_USBTEMP_LIMIT_CHARGER == val) {
		chip->smart_charge_user = SMART_CHARGE_USER_USBTEMP;
		//vooc
		//chip->limits.usbtemp_input_current_vooc_ma_normal = USBTEMP_LIMIT_INPUT_CURRENT;
		//chip->cool_down_done = true;
		//chip->chg_ctrl_by_cool_down = true;
		//svooc
		if (oppo_vooc_get_vooc_multistep_adjust_current_support() == false)
			chip->usbtemp_cool_down = 1;
		else
			chip->usbtemp_cool_down = 3;
	}
	if (SMART_EXIT_USBTEMP_LIMIT_CHARGER == val) {
		chip->smart_charge_user = SMART_CHARGE_USER_OTHER;
	}
}
