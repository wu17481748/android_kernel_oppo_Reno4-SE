...
static void bq27541_parse_dt(struct chip_bq27541 *chip)
{
	...
	of_property_read_u32(node, "qcom,sha1_key_index", &chip->sha1_key_index);
}
...
static int seal(void)
{
	...
	pr_err("%s, oppo_chg_gauge_seal_unseal_fail enter\n",__func__);
	oppo_chg_gauge_seal_unseal_fail(OPPO_GAUGE_SEAL_FAIL);
	return 0;
}
...
out:
	...
	if (i == SEAL_POLLING_RETRY_LIMIT) {
		pr_err("%s bq27541 failed\n",__func__);
		oppo_chg_gauge_seal_unseal_fail(OPPO_GAUGE_UNSEAL_FIAL);
		return 0;
	} else {
		return 1;
	}
