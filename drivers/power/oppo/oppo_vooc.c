...
int oppo_vooc_get_smaller_battemp_cooldown(int ret_batt, int ret_cool){
	...
	if( ret_batt > 0 && ret_batt < ARRAY_SIZE(multistepCurrent)+1
		...
		oppo_chg_debug_get_cooldown_current(ret_batt_current, ret_cool_current);
		...
		if(ret_cool > 0)
		{
			if(ret_cool_current < ret_batt_current)
			{
				...
				oppo_chg_debug_set_cool_down_by_user(1);
			}
			else
			{
				//clear flag cool down by user
				oppo_chg_debug_set_cool_down_by_user(0);
			}
		}
...
static void oppo_vooc_fastchg_func(struct work_struct *work)
{
	...
	int volt = oppo_chg_get_batt_volt();
	int temp = oppo_chg_get_chg_temperature();
	int soc = oppo_chg_get_soc();
	int current_now = oppo_chg_get_icharging();
	int chg_vol = oppo_chg_get_charger_voltage();
	int remain_cap = 0;
...
void oppo_vooc_get_vooc_chip_handle(struct oppo_vooc_chip **chip)
{
    *chip = g_vooc_chip;
}