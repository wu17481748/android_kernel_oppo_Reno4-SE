...
extern int oppo_vooc_asic_hwid_check(struct oppo_vooc_chip *chip);

int get_vooc_mcu_type(struct oppo_vooc_chip *chip)
{
	int mcu_hwid_type = OPPO_VOOC_MCU_HWID_UNKNOW;

	if(chip == NULL){
		chg_err("oppo_vooc_chip is not ready, enable stm8s\n");
		return OPPO_VOOC_MCU_HWID_STM8S;
	}
	mcu_hwid_type = oppo_vooc_asic_hwid_check(chip);
	if (mcu_hwid_type != OPPO_VOOC_ASIC_HWID_NON_EXIST) {
...
int oppo_vooc_asic_hwid_check(struct oppo_vooc_chip *chip)
{
	int rc;
	static int asic_hwid_type = OPPO_VOOC_MCU_HWID_UNKNOW;
	struct device_node *node = NULL;

	if(asic_hwid_type != OPPO_VOOC_MCU_HWID_UNKNOW) {
	...
	if(chip == NULL){
		...
		asic_hwid_type = OPPO_VOOC_ASIC_HWID_NON_EXIST;
	...
	if(gpio_get_value(chip->vooc_gpio.vooc_asic_id_gpio) == 1){
		...
		asic_hwid_type = OPPO_VOOC_ASIC_HWID_RK826;
	...
	if(gpio_get_value(chip->vooc_gpio.vooc_asic_id_gpio) == 1){
		...
		asic_hwid_type = OPPO_VOOC_ASIC_HWID_RT5125;
		return OPPO_VOOC_ASIC_HWID_RT5125;
	}

	chg_debug("it is  op10\n");
	asic_hwid_type = OPPO_VOOC_ASIC_HWID_OP10;
...
static void delay_reset_mcu_work_func(struct work_struct *work)
{
	...
	struct oppo_vooc_chip *chip = container_of(dwork,
		struct oppo_vooc_chip, delay_reset_mcu_work);
