...
static void kpd_set_vol_key_state(int key, int key_val)
{
	...
	if (key_val) {
		...
		if(start_timer_last != 0){
			...
			{
#ifdef VENDOR_EDIT
/*You.Li@BSP.Kernel.Stability, 2020-7-1, Modify to disable forece dump in US version*/
#ifdef CONFIG_OPPO_US_VERSION
				pr_err("US verison, disable force dump\n");
				return;
#endif
#endif/*VENDOR_EDIT*/
...
/* for keymap handling */
#ifdef VENDOR_EDIT
/*shfian@BSP.TP, 2019/12/10, Add for disable kpd irq handler*/
...
#ifdef VENDOR_EDIT
/*shifan@BSP.TP, 2019/12/10, Add for disable kpd irq handler*/
/*
static void kpd_keymap_handler(unsigned long data)
{
	u16 i, j;
