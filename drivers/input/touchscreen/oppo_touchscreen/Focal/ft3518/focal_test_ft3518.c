 ...
 * VENDOR_EDIT
 * File           : focaltech_test_ft3518.c
 * Description: Source file for focal 3518
 * Version   : 1.0
 * Date                : 2020/05/13
...
static int fts_test_bus_read(u8 *cmd, int cmdlen, u8 *data, int datalen)
{
    ...
    ret = touch_i2c_read(fts_data->client, (char *)cmd, cmdlen, (char *)data, datalen);
    if (ret < 0)
        return ret;
    else
        return 0;
...
static int fts_test_bus_write(u8 *writebuf, int writelen)
{
    ...
    ret = touch_i2c_write(fts_data->client, (char *)writebuf, writelen);
    if (ret < 0)
        return ret;
    else
        return 0;
