 ...
 * VENDOR_EDIT
 * File           : focaltech_drivers_ft3518.c
 * Description: focal 3518 driver
 * Version   : 1.0
 * Date                : 2020/05/13
...
static bool fts_fwupg_check_flash_status(struct fts_ts_data *ts_data, u16 flash_status, int retries, int retries_delay)
{
    ...
    for (i = 0; i < retries; i++) {
        ...
        ret = touch_i2c_read(ts_data->client, &cmd , 1, val, 2);
...
static int fts_fwupg_enter_into_boot(struct fts_ts_data *ts_data)
{
    ...
    do {
        ...
        /*read boot id*/
        cmd = FTS_CMD_START;
        ret = touch_i2c_write(ts_data->client, &cmd, 1);
        if (ret < 0) {
        ...
        cmd = FTS_CMD_READ_ID;
        ret = touch_i2c_read(ts_data->client, &cmd, 1, id, 2);
        if (ret < 0) {
...
int fts_fwupg_ecc_cal_tp(struct fts_ts_data *ts_data, u32 saddr, u32 len)
{
    ...
    ret = touch_i2c_write(ts_data->client, wbuf, 1);
    if (ret < 0) {
    ...
    ret = touch_i2c_write(ts_data->client, wbuf, 7);
    if (ret < 0) {
    ...
    ret = touch_i2c_read(ts_data->client, wbuf, 1, &ecc, 1);
    if (ret < 0) {
...
static int fts_upgrade(struct fts_ts_data *ts_data, u8 *buf, u32 len)
{
    ...
    cmd[2] = (len >> 8) & 0xFF;
    cmd[3] = (len) & 0xFF;
    ret = touch_i2c_write(ts_data->client, cmd, 4);
    if (ret < 0) {
    ...
    cmd[0] = FTS_CMD_RESET;
    ret = touch_i2c_write(ts_data->client, &cmd[0], 1);
    if (ret < 0) {
...
static int fts_get_chip_info(void *chip_data)
{
    ...
    touch_i2c_read(ts_data->client, &cmd, 1, id, 2);
    TPD_INFO("read boot id:0x%02x%02x", id[0], id[1]);
    if ((id[0] == FTS_VAL_BL_ID) && (id[1] == FTS_VAL_BL_ID2)) {
        return 0;
    }
...
static void fts_read_fod_info(struct fts_ts_data *ts_data)
{
    ...
    ret = touch_i2c_read(ts_data->client, &cmd, 1, val, FTS_REG_FOD_INFO_LEN);
    if (ret < 0) {
...
static u8 fts_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
    ...
    memset(buf, 0, FTS_MAX_POINTS_LENGTH);
    ...
    ret = touch_i2c_read(ts_data->client, &cmd, 1, &buf[0], FTS_POINTS_ONE);
    if (ret < 0) {
...
//TODO    SET_BIT(result_event, IRQ_FINGERPRINT);
...
static int fts_get_touch_points(void *chip_data, struct point_info *points, int max_num)
{
    ...
    if (buf[FTS_POINTS_ONE - 1] == 0xFF)
        ...
        ret = touch_i2c_read(ts_data->client, &cmd, 1, &buf[FTS_POINTS_ONE], FTS_POINTS_TWO);
    if (ret < 0) {
...
static int fts_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
    ...
    ret = touch_i2c_read(ts_data->client, &cmd, 1, &buf[2], FTS_GESTURE_DATA_LEN - 2);
    if (ret < 0) {
...
static void fts_register_info_read(void *chip_data, uint16_t register_addr, uint8_t *result, uint8_t length)
{
    ...
    touch_i2c_read(ts_data->client, &addr, 1, result, length);
}
