...
struct mtk_panel_funcs {
	...
	int (*ext_param_get)(struct mtk_panel_params *ext_para, unsigned int mode);
	 ...
	 */
#if USE_GCE_WRITE_CMD
	int (*doze_enable_start)(void *dsi, dcs_write_gce cb, void *handle);
#else
	int (*doze_enable_start)(struct drm_panel *panel);
#endif

#if USE_GCE_WRITE_CMD
	int (*doze_enable)(void *dsi, dcs_write_gce cb, void *handle);
#else
	int (*doze_enable)(struct drm_panel *panel);
#endif

#if USE_GCE_WRITE_CMD
	int (*doze_enable_end)(void *dsi, dcs_write_gce cb, void *handle);
#else
	 ...
	 */
#if USE_GCE_WRITE_CMD
	int (*doze_disable)(void *dsi, dcs_write_gce cb, void *handle);
#else
	 ...
	 */
#if USE_GCE_WRITE_CMD
	int (*doze_post_disp_on)(void *dsi, dcs_write_gce cb, void *handle);
#else
	int (*doze_post_disp_on)(struct drm_panel *panel);
#endif
	/**
	* @doze_post_disp_off:
	* @doze_post_disp_on to turn off panel.
	*
	*/
#if USE_GCE_WRITE_CMD
	int (*doze_post_disp_off)(void *dsi, dcs_write_gce cb, void *handle);
#else
	 ...
	 */
#if USE_GCE_WRITE_CMD
	int (*doze_area)(void *dsi, dcs_write_gce cb, void *handle);
#else
...
#else
	int (*doze_area_set)(struct drm_panel *panel);
#endif
...
/**
	 * @panel display off:
	 *
	 * Send the panel display off in command here.
	 */
#if USE_GCE_WRITE_CMD
	int (*panel_disp_off)(void *dsi, dcs_write_gce cb, void *handle);
#else
	int (*panel_disp_off)(struct drm_panel *panel);
#endif
#endif
	...
	int (*set_aod_light_mode)(void *dsi, dcs_write_gce cb, void *handle, unsigned int level);
	bool (*panel_no_cv_switch)(struct drm_panel *panel);
	#endif
	#ifdef VENDOR_EDIT
	...
	#endif
	#ifdef VENDOR_EDIT
	/*shangruofan@PSW.MM.LCD.Display.Stability, 2020/4/4, add for esd backlight*/
