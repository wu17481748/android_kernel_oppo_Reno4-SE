...
#ifdef VENDOR_EDIT
/* liping-m@PSW.MM.DisplayDriver.Lcd, 2019/11/1, add for ffl backlight */
extern struct drm_device* get_drm_device(void);
extern bool oppo_mtk_drm_get_hbm_state(void);
				     ...
				     uint64_t val)
{
	...
	for (i = 0; i < CRTC_PROP_MAX; i++) {
		if (private->crtc_property[index][i] == property) {
			...
			DDPINFO("%s crtc:%d set property:%s %d\n",
					__func__,index, property->name,
					(unsigned int)val);
...
int oppo_mtk_drm_setbacklight(struct drm_crtc *crtc, unsigned int level)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct cmdq_pkt *cmdq_handle;
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
...
int oppo_mtk_drm_ffl_backlight(unsigned int level)
{
...
int oppo_mtk_drm_sethbm(struct drm_crtc *crtc, unsigned int hbm_mode)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct cmdq_pkt *cmdq_handle;
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
...
int oppo_mtk_drm_setcabc(struct drm_crtc *crtc, unsigned int cabc_mode)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct cmdq_pkt *cmdq_handle;
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
...
/*liwei.a@PSW.MM.LCD.Display.Stability, 2019/10/16, add for mipi clk change*/
int oppo_drm_set_safe_mode(struct drm_crtc *crtc, unsigned int level)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct cmdq_pkt *cmdq_handle;
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
...
/* liwei.a@PSW.MM.DisplayDriver.Stability, 2019/12/5, delete for cmdq timeout error */
	mutex_lock(&mtk_crtc->lock);
...
static int ffl_set_worker_kthread(void *data)
{
	...
	while (1) {
		ret = wait_event_interruptible(mtk_crtc->ffl_task_wq, atomic_read(&ffl_task_wakeup));
		atomic_set(&ffl_task_wakeup, 0);
		DDPINFO("[fflset]ffl_set_worker_kthread\n");
		...
		if (ffl_set_mode == FFL_TRIGGLE_CONTROL) {
			for (index = FFL_START_LEVEL; index <= FFL_END_LEVEL; index = index + FFL_UPRATE) {
				if (ffl_set_mode != FFL_TRIGGLE_CONTROL || oppo_mtk_drm_get_hbm_state()) {
					break;
				}
				if ((index > 1) && ffl_display_ready
					&& (ffl_backlight_backup != 0)) {
					oppo_mtk_drm_ffl_backlight(index);
				} else {
					break;
				}
				msleep(6);
			...
			if ((ffl_backlight_backup > 1)
				&& (ffl_set_mode != FFL_EXIT_FULLY_CONTROL)
				&& ffl_display_ready) {
				oppo_mtk_drm_ffl_backlight(ffl_backlight_backup);
			}
		}
...
void ffl_set_init(struct mtk_drm_crtc *mtk_crtc)
{
    mtk_crtc->ffl_set_task
        = kthread_create(ffl_set_worker_kthread, mtk_crtc,"FFL_SET");
	init_waitqueue_head(&mtk_crtc->ffl_task_wq);
	wake_up_process(mtk_crtc->ffl_set_task);
	DDPINFO("[fflset]ffl_set_init\n");
}
...
void ffl_set_enable(struct mtk_drm_crtc *mtk_crtc, unsigned int enable)
{
	if (enable == FFL_TRIGGLE_CONTROL) {
		...
		wake_up_interruptible(&mtk_crtc->ffl_task_wq);
		DDPINFO("[fflset]enable ffl_set\n");
...
#ifdef VENDOR_EDIT
/* liwei.a@PSW.MM.DisplayDriver.Stability, 2020/07/28, add for aod backlight getting effective right now*/
...
int mtk_drm_aod_setbacklight(struct drm_crtc *crtc, unsigned int level)
{
	...
	if (output_comp->funcs && output_comp->funcs->io_cmd)
		output_comp->funcs->io_cmd(output_comp, cmdq_handle, AOD_BL, &level);
...
static int mtk_drm_crtc_set_panel_hbm(struct drm_crtc *crtc, bool en)
{
	...
	/* shangruofan@PSW.MM.DisplayDriver.Stability, 2020/08/01, add for DC */
	bool is_frame_mode;
	#endif
	bool state = false;
	...
	#ifdef OPPO_DISPLAY_LCD_20075
	if(oppo_dc_enable && oppo_display_brightness < OPPO_DC_BACKLIGHT_THRESHOLD && en == 1){
		is_frame_mode = mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base);
		cmdq_handle =
			cmdq_pkt_create(mtk_crtc->gce_obj.client[CLIENT_DSI_CFG]);
		if (is_frame_mode) {
		...
		comp->funcs->io_cmd(comp, cmdq_handle, DC_BACKLIGHT, &state);
		if (is_frame_mode)
				cmdq_pkt_set_event(cmdq_handle,
                                  mtk_crtc->gce_obj.event[EVENT_CABC_EOF]);
...
#ifndef VENDOR_EDIT
/* liwei.a@PSW.MM.DisplayDrvier.FP, 2019/12/16, delelte for splash high light in fingerprint scene*/
...
int notify_display_fpd(bool mode) {
	if (mode) {
		if (power_mode == 3) {
			...
			doze_rec_fpd = false;
			ds_rec_fpd = false;
			pr_info("[LW][fpdnotify] fp up reset flag\n");
		}
	}
...
#ifdef VENDOR_EDIT
/** LiPing-M@PSW.MM.Display.LCD.Machine, 2019/11/15,add for dc backlight **/
		if ((oppo_dc_enable != oppo_dc_enable_real) && (crtc_id == 0)) {
			oppo_dc_enable_real = oppo_dc_enable;
			oppo_dc_set = true;
			oppo_mtk_drm_setbacklight(crtc, ffl_backlight_backup);
		}
#endif
	}
}
...
void mtk_crtc_start_trig_loop(struct drm_crtc *crtc)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		if (mtk_drm_lcm_is_connect()) {
			if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->funcs
                && mtk_crtc->panel_ext->funcs->panel_no_cv_switch){
				if(mtk_crtc->panel_ext->funcs->panel_no_cv_switch){
					cmdq_pkt_wfe(cmdq_handle,
						mtk_crtc->gce_obj.event[EVENT_TE]);
				}
			} else {
				/* wait 3-TE to meet 10fps request in doze mode */
				cmdq_pkt_wfe(cmdq_handle,
					mtk_crtc->gce_obj.event[EVENT_TE]);
				cmdq_pkt_wfe(cmdq_handle,
					mtk_crtc->gce_obj.event[EVENT_TE]);
...
void mtk_crtc_stop_sodi_loop(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_drm_private *priv = NULL;
	/* Shang.RuoFan@PSW.MM.DisplayDriver.Stability, 2020/06/10,add for check sodi_loop_cmdq_handle */
	if (mtk_crtc->sodi_loop_cmdq_handle == NULL) {
		DDPPR_ERR("%s:%d, sodi_loop already stopped\n",
			   __func__, __LINE__);
		return;
    }
...
void mtk_crtc_config_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params &&
		...
		cfg.vrefresh = mtk_crtc->panel_ext->params->dyn_fps.vact_timing_fps;
	else
	cfg.vrefresh = crtc->state->adjusted_mode.vrefresh;
	cfg.bpc = mtk_crtc->bpc;
...
/**shangruofan@PSW.MM.Display.LCD, 2020/07/14,add for corner **/
#ifndef VENDOR_EDIT
	if (panel_ext) {
#else
	if (panel_ext && panel_ext->corner_pattern_tp_size) {
#endif
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM)) {
		...
		#ifdef VENDOR_EDIT
		/* liwei.a@PSW.MM.DisplayDriver.FP, 2019/11/10, add for fingerprint*/
		/* liwei.a@PSW.MM.DsiplayDriver.FP, 2019/11/29, modify that set hbm once dimlayer occurs*/
		DDPINFO("[LW] hbm_en = %d\n", hbm_en);
		#ifdef VENDOR_EDIT
		/* zhangjianbin@PSW.MM.DisplayDriver.FP, 2020/1/10, add a new lcd*/
		if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->funcs
			...
			if(mtk_crtc->panel_ext->funcs->panel_no_cv_switch){
				if ((hbm_en > 0) && mtk_crtc_is_frame_trigger_mode(crtc)) {
					DDPINFO("[MYFP]set hbm on\n");
					mtk_drm_crtc_set_panel_hbm(crtc, 1);
					...
					if (oppo_fp_notify_down_delay && (hbm_en & 0x2) > 0) {
						...
						DDPMSG("%s: write pf_idx to pf0:%u\n",
							__func__, pf_idx);
							cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
								addr, pf_idx, ~0);
						#endif
							...
							DDPINFO("[MYFP]set hbm off\n");
							mtk_drm_crtc_set_panel_hbm(crtc, 0);
				}
			}
		} else {
			if ((hbm_en > 0) && !mtk_crtc_is_frame_trigger_mode(crtc)) {
	...
	//if (!is_doze) {
	DDPINFO("[MYFP]in normal mode, oppo_fp_notify_down_delay=%d, oppo_fp_notify_up_delay=%d\n", oppo_fp_notify_down_delay, oppo_fp_notify_up_delay);
	/* hbm wait && send fp 1*/
	#ifdef VENDOR_EDIT
	/* zhangjianbin@PSW.MM.DisplayDriver.FP, 2020/1/10, add a new lcd*/
	if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->funcs
			...
			const struct mtk_crtc_path_data *path_data)
{
	...
	#ifdef VENDOR_EDIT
	/*
	* liping-m@PSW.MM.Display.LCD, 2019/11/19,
	* add for face fill light node
	*/
	ffl_set_init(mtk_crtc);
	#endif /* VENDOR_EDIT */
			...
			unsigned int userdata)
{
	...
	#ifdef VENDOR_EDIT
	/* liwei.a@PSW.MM.DisplayDriver.DMClock, 2019/10/31, add for dynamic mipi clock*/
	oppo_drm_set_safe_mode(crtc, 0);  //send safe fail
	#endif
	...
	#ifdef VENDOR_EDIT
	/* liwei.a@PSW.MM.DisplayDriver.DMClock, 2019/10/31, add for dynamic mipi clock*/
	oppo_drm_set_safe_mode(crtc, 1);  //send safe on
	#endif
done:
	DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
