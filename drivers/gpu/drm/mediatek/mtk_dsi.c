	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (!(dsi->mipi_hopping_sta && cur_panel_params &&
		adjust_panel_params &&
				     ...
				     const struct mipi_dsi_msg *msg, u8 flag)
{
	...
	#ifdef VENDOR_EDIT
	#if defined(CONFIG_MACH_MT6885)
	/*liwei.a@PSW.MM.Display.Stability, modify just for 6885 aod, 2020/06/03*/
	mtk_dsi_mask(dsi, DSI_INTSTA, 2, 0);
	#endif
	#endif /*VENDOR_EDIT*/
	mtk_dsi_start(dsi);
	...
	/*liwei.a@PSW.MM.Display.Stability, modify just for 6885 aod, 2020/06/03*/
		...
		while (loop_cnt < 100 * 1000) {
			tmp = readl(dsi->regs + DSI_INTSTA);
			#ifndef VENDOR_EDIT
			/* liwei.a@PSW.MM.DisplayDriver.Stability, 2019/12/08, modify to fix doze to off cost too much time*/
			if ((tmp & 2))
			#else
			if (!(tmp & DSI_BUSY))
			#endif
		...
		#ifndef VENDOR_EDIT
		/* liwei.a@PSW.MM.DisplayDriver.Stability, 2019/12/08, modify to fix doze to off cost too much time*/
		DDPINFO("%s wait cmd done\n", __func__);
		#else
		if (loop_cnt < 100 * 1000)
			DDPINFO("%s wait not busy done\n", __func__);
		else
			DDPPR_ERR("%s wait not busy fail\n", __func__);
		#endif
		mtk_dsi_mask(dsi, DSI_INTSTA, 2, 0);
	#endif
	#endif /*VENDOR_EDIT*/
	...
	if (!mtk_dsi_wait_idle(dsi, flag, 2000, NULL)){
		DDPINFO("%s mtk_dsi_wait_idle timeout\n", __func__);
		return -ETIME;
	}
	else{
		DDPINFO("%s mtk_dsi_wait_idle ok\n", __func__);
		return 0;
	}
}
...
//1.fps change index
	...
	if (fps_chg_index & DYNFPS_DSI_MIPI_CLK) {
		...
		if (!need_send_cmd) {
			...
			if (dsi->mipi_hopping_sta) {
				DDPINFO("%s,V timing changed mipi_clk_change_sta\n", __func__);
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_VFP_IDLE_MODE:
		...
		if(vfp_low_power) {
			DDPINFO("DSI_VFP_IDLE_MODE,vfp_low_power=%d\n",vfp_low_power);
			mtk_dsi_porch_setting(comp, handle, DSI_VFP,
					vfp_low_power);
		}
		}
