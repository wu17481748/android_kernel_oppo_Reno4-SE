...
struct devinfo_data {
	...
	struct iio_channel *auxadc3_channel;
};

static int sim_detect_gpio = -1;
...
static int device_node_read_sim_detect(struct seq_file *s, void *v) {
    int sim_detect_value = -1;
    sim_detect_value = gpio_get_value(sim_detect_gpio);
    seq_printf(s, "%d\n", sim_detect_value);
    return 0;
}

static int device_node_for_sim_detect_open(struct inode *inode, struct file *file) {
    return single_open(file, device_node_read_sim_detect, NULL);
}

static const struct file_operations device_node_for_sim_detect = {
    .open = device_node_for_sim_detect_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int sim_card_detect_init(struct devinfo_data *const devinfo_data) {
    int ret = 0;
    struct device_node *np = NULL;
    struct proc_dir_entry *p = NULL;

    np = devinfo_data->devinfo->dev.of_node;
    devinfo_data->sim_detect = of_get_named_gpio(np, "Hw,sim_det", 0);

    if (devinfo_data->sim_detect < 0) {
        DEVINFO_ERR("sim detect gpio not specified\n");
        ret = -1;
        goto err;
    }

    sim_detect_gpio = devinfo_data->sim_detect;
    p = proc_create("sim_detect", 0644, NULL, &device_node_for_sim_detect);
    if(!p) {
        pr_err("proc create sim detect failed\n");
        ret = -1;
        goto err;
    }
err:
...
static int subboard_init(struct devinfo_data *const devinfo_data) {
	...
	if(OPPO_19040 == get_project()) {
		devinfo_data->auxadc3_channel = kzalloc(sizeof(*devinfo_data->auxadc3_channel), GFP_KERNEL);
		if (!devinfo_data->auxadc3_channel) {
			pr_err("subboard_init kzalloc failed!\n");
			return -ENOMEM;
		}

		devinfo_data->auxadc3_channel = iio_channel_get(&devinfo_data->devinfo->dev, "auxadc3-name");
		ret = IS_ERR(devinfo_data->auxadc3_channel);
		if (ret) {
			pr_err("[%s] fail to get auxadc iio ch0: %d\n", __func__, ret);
			kfree(devinfo_data->auxadc3_channel);
			devinfo_data->auxadc3_channel = NULL;
			return -1;
		}
	} else {
	...
	if (devinfo_data->sub_mainboard_info.manufacture == NULL) {
		...
		devinfo_data->sub_mainboard_info.version = NULL;
		if(OPPO_19040 == get_project()) {
			kfree(devinfo_data->auxadc3_channel);
			devinfo_data->auxadc3_channel = NULL;
		}
...
static int subboard_verify(struct devinfo_data *const devinfo_data)
{
	...
	if(OPPO_19040 == get_project()) {
		ret = iio_read_channel_processed(devinfo_data->auxadc3_channel, &val);
		if (ret < 0) {
			pr_err("Busy/Timeout, IIO ch read failed %d\n", ret);
			return ret;
		}
		val = (val * 1500) >> 12;
		pr_err("subboard_verify adc_val is %d\n", val);
	} else {
	...
	switch(get_project()) {
		...
		case OPPO_19165:
		case OPPO_19166:
			if ((id1 == 1) && (id2 == 1) && (operator == OPERATOR_19165_All_BAND || operator == OPERATOR_19166_MOBILE)) {
				snprintf(devinfo_data->sub_mainboard_info.manufacture, INFO_BUF_LEN, "sub-evt1");
			} else if((id1 == 0) && (id2 == 1) && (operator == OPERATOR_19165_All_BAND || operator == OPERATOR_19166_MOBILE)) {
				snprintf(devinfo_data->sub_mainboard_info.manufacture, INFO_BUF_LEN, "sub-evt2");
			}else {
				snprintf(devinfo_data->sub_mainboard_info.manufacture, INFO_BUF_LEN, "rf-unmatch");
			}
			break;
	...
	if(OPPO_19040 == get_project()) {} else {
...
static int devinfo_probe(struct platform_device *pdev)
{
	...
	sim_card_detect_init(devinfo_data);
